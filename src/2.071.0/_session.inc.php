...
echo ("<div><strong>Etat des variables a la ligne : ".__LINE__." dans le fichier : ".__FILE__."</strong><br />\n");
...
require_once ("config/config_systeme.inc.php");
...
require_once ($DIR."_recherche_perso.lib.php");			// Fonctions li�es aux recherches personnalis�e
...
// Classes de modeles de messages charg�es dynamiquement
...
if (!isset($_SESSION['profils'])) {
	...
	// Chargement des profils en donnee de session
...
if(isset($_INTERFACE['MUST_BE_LOGIN'])){
	...
	//CAS 1
	//$_INTERFACE['MUST_BE_LOGIN'] est d�fini
	//$_PAGE['MUST_BE_LOGIN'] est d�fini
		if($_PAGE['MUST_BE_LOGIN']){page_accessible_a_tous();}
	}else{
	//CAS 2
	//$_INTERFACE['MUST_BE_LOGIN'] est d�fini
	//$_PAGE['MUST_BE_LOGIN'] n'est pas d�fini
		if($_INTERFACE['MUST_BE_LOGIN']){page_accessible_a_tous();}
	}
}else{
	if(isset($_PAGE['MUST_BE_LOGIN'])){
	//CAS 3
	//$_INTERFACE['MUST_BE_LOGIN'] n'est pas d�fini
	//$_PAGE['MUST_BE_LOGIN'] est d�fini
		if($_PAGE['MUST_BE_LOGIN']){page_accessible_a_tous();}
	}else{
	//CAS 4
...
// Verification que l'ID MAGASIN existe et est param�tr� dans la configuration, sinon message d'alerte et destruction de la session pour �viter un acc�s forc� en reloadant la page.
							...
							<table width="100%" cellpadding=0 cellspacing=0 border=0 align="center">
								...
								<tr>
									<td colspan="2" style="text-align: center;">
										<p>Le magasin appel� pour la connexion client ne semble pas �tre correctement configur�.<br />
											Le magasin attribu� � la connexion client est le <strong>n�<?php echo $ID_MAGASIN; ?></strong>, or il ne semble pas �tre d�fini dans la configuration des magasins VPC.<br /><br />
											Veuillez v�rifier les configurations (vous pouvez d�finir un magasin VPC dans l'inteface admninistrateur > Points de vente,<br />
											un m�me magasin ne peut �tre d�fini comme VAC et VPC sur la m�me ligne)
										</p>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<input type="button" onclick="javascript: history.go(-1)" value="Retour � la page pr�c�dente" />
									</td>
								</tr>
								<tr>
