...
if (isset($_REQUEST["duree_avant_purge_annule_".$_REQUEST["id_type_doc"]])) {
	maj_configuration_file ("_doc_".strtolower($code_doc).".config.php", "maj_line", "\$DUREE_AVANT_PURGE_ANNULE_".strtoupper($code_doc)." =", "\$DUREE_AVANT_PURGE_ANNULE_".strtoupper($code_doc)." = ".$_REQUEST["duree_avant_purge_annule_".$_REQUEST["id_type_doc"]].";	// Délai avant la suppression des docs annulés", $LIBDIR."documents/");
}
...
maj_configuration_file ("_doc_".strtolower($code_doc).".config.php", "maj_line", "\$DEVIS_CLIENT_AUTO_GENERE	=", "\$DEVIS_CLIENT_AUTO_GENERE	= \"".$dev_auto."\";	// code doc du type de doc créé à la validation du devis", $LIBDIR."documents/");
maj_configuration_file ("_doc_".strtolower($code_doc).".config.php", "maj_line", "\$DEVIS_CLIENT_OPTION_GENERE	= array(", "\$DEVIS_CLIENT_OPTION_GENERE	= array(".implode(",",$dev_option).");	// code doc des types de doc en option à la validation du devis", $LIBDIR."documents/");
