<?php
// *************************************************************************************************************
// CONFIGURATION DES DONNï¿½ES tarifs
// *************************************************************************************************************
...
//mise ï¿½ jour des donnï¿½es transmises

if (isset($_REQUEST["CALCUL_VAA"])) {
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$CALCUL_VAA =", "\$CALCUL_VAA = \"".$_REQUEST["CALCUL_VAA"]."\"; //PA utilisï¿½ pour calcul de Valeur d'Achat Actuelle", $CONFIG_DIR);
}
...
if (isset($_REQUEST["DUREE_VALIDITE_PAF_an"])) {
	...
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$DUREE_VALIDITE_PAF =", "\$DUREE_VALIDITE_PAF = ".$duree_val."; //durï¿½e validitï¿½ PA fournisseur", $CONFIG_DIR);
}

if (isset($_REQUEST["CALCUL_VAS"])) {
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$CALCUL_VAS =", "\$CALCUL_VAS = \"".$_REQUEST["CALCUL_VAS"]."\"; //PA utilisï¿½ pour calcul de Valeur d'Achat Stockï¿½e", $CONFIG_DIR);
}
...
if (isset($_REQUEST["use_formules"]) && $_REQUEST["use_formules"] == "1") {
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$USE_FORMULES					=", "\$USE_FORMULES					= 1;								// utilisation du gï¿½nï¿½rateur de formule (sinon, tarifs dï¿½finis arbitrairements)", $CONFIG_DIR);
} else {
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$USE_FORMULES					=", "\$USE_FORMULES					= 0;								// utilisation du gï¿½nï¿½rateur de formule (sinon, tarifs dï¿½finis arbitrairements)", $CONFIG_DIR);
}
...
if (isset($_REQUEST["defaut_arrondi"])) {
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$DEFAUT_ARRONDI				=", "\$DEFAUT_ARRONDI				= \"".$_REQUEST["defaut_arrondi"]."\"; 						// Par dï¿½faut, l'arrondi est PRO, SUP, ou INF", $CONFIG_DIR);
}

if (isset($_REQUEST["defaut_arrondi_pas"]) && is_numeric($_REQUEST["defaut_arrondi_pas"])) {
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$DEFAUT_ARRONDI_PAS		=", "\$DEFAUT_ARRONDI_PAS		= ".$_REQUEST["defaut_arrondi_pas"].";							// Nombre de dï¿½cimales par dï¿½faut pour l'arrondi d'un article", $CONFIG_DIR);
}

if (isset($_REQUEST["tarifs_nb_decimales"]) && is_numeric($_REQUEST["tarifs_nb_decimales"])) {
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$TARIFS_NB_DECIMALES =", "\$TARIFS_NB_DECIMALES = ".$_REQUEST["tarifs_nb_decimales"].";									// Nombre de dï¿½cimales affichï¿½es pour les tarifs", $CONFIG_DIR);
}

if (isset($_REQUEST["defaut_app_tarifs_client"])) {
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$DEFAUT_APP_TARIFS_CLIENT 			=", "\$DEFAUT_APP_TARIFS_CLIENT 			= \"".$_REQUEST["defaut_app_tarifs_client"]."\";	// Affichage des tarifs HT ou TTC par dï¿½faut pour les clients", $CONFIG_DIR);
}

if (isset($_REQUEST["defaut_app_tarifs_fournisseur"])) {
	maj_configuration_file ("config_generale.inc.php", "maj_line", "\$DEFAUT_APP_TARIFS_FOURNISSEUR	=", "\$DEFAUT_APP_TARIFS_FOURNISSEUR	= \"".$_REQUEST["defaut_app_tarifs_fournisseur"]."\";		// Affichage des tarifs HT ou TTC par dï¿½faut pour les fournisseurs", $CONFIG_DIR);
}
